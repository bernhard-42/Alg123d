...
for loc in GridLocations(x_spacing=0, y_spacing=lego_unit_size, x_count=1, y_count=2):
    plan += Rectangle(width=block_length, height=ridge_width) @ loc

for loc in GridLocations(lego_unit_size, 0, pip_count, 1):
    plan += Rectangle(width=ridge_width, height=block_width) @ loc
...
for loc in GridLocations(
    ...
    plan += Circle(support_outer_diameter / 2) @ loc
    plan -= Circle(support_inner_diameter / 2) @ loc
...
for loc in Locations((0, 0, S.max_vertex(lego).Z)):
