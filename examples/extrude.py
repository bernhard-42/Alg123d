...
for plane in Planes(multiple.faces()):
    for loc in GridLocations(5, 5, 2, 2):
        t += plane * loc * Text("Ω", font_size=3, align=Align.CENTER)
...
for loc in Planes(single_multiple.faces()):
    single_multiple -= loc * extrude(rect.faces()[0], amount=-2)

# Non-planar surface
non_planar = Rot(x=90) * Cylinder(10, 20, align=(Align.CENTER, Align.MIN, Align.CENTER))
...
key_cap += extrude_until(Pos(z=4 * MM) * ribs, key_cap)
...
key_cap += extrude(plane * socket, amount=3.5 * MM)

if "show_object" in locals():
    show_object(Pos(-15, 0, 0) * simple, name="simple pending extrude")
    show_object(Pos(20, 10, 0) * both, name="simple both")
    show_object(Pos(0, -20, 0) * multiple, name="multiple pending extrude")
    show_object(Pos(0, 20, 0) * single_multiple, name="single multiple")
    show_object(Pos(20, -10, 0) * non_planar, name="non planar")
